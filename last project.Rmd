...
```{r}
library(readr)    # To read Excel files (.xlsx)
...
install.packages("ggpmisc") #Installs the ggpmisc package, which provides extra tools for annotating ggplots — for example, adding regression equations, R² values, and correlation coefficients directly on plots.
...
```
```{r}
par(mfrow = c(2,2)) # divides the plotting area into a 2×2 grid (so four plots appear together).
plot(lm_fit) # draws the diagnostic plots for the regression model (Residuals, Q–Q plot, Scale–Location, and Residuals vs Leverage).
...
library(caret) # Building and training machine learning models (like linear regression, decision trees, etc.)
               ...
               # Calculating model accuracy, RMSE, and R²
...
```{r}
set.seed(42) # Sets a random seed so results are reproducible (you get the same results each time you run it).
train_control <- trainControl(method = "repeatedcv", number = 10, repeats = 3) # Creates the cross-validation setup.
                                                                               # method = "repeatedcv" → use repeated k-fold cross-validation.
                                                                               # number = 10 → divide the data into 10 folds.
                                                                               # repeats = 3 → repeat the 10-fold process three times for more stable accuracy.

caret_lm <- train(PM25 ~ PM10,
data = df,
method = "lm",
trControl = train_control) # Trains a linear regression model (lm) using caret,
                           # where PM25 is predicted from PM10,
                           # and the training process follows the train_control (cross-validation setup).
caret_lm$results           
# Displays the model performance metrics — such as:
                           # RMSE → Root Mean Square Error (lower is better)  
                           # R² → how well PM10 explains PM2.5 (higher is better)
